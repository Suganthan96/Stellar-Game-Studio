  ...
  return {
    ...
    signAuthEntry: walletSigner.signAuthEntry
      ...
      ? (async (entryXdr: string, opts?: { networkPassphrase?: string }) => {
          ...
          // Freighter returns the entire signed auth entry XDR, not just the signature.
          // We need to extract the raw 64-byte signature from it.
          try {
            console.log('[sdkSigner] parsing signedAuthEntry XDR, length:', result.signedAuthEntry.length);
            const signedEntry = xdr.SorobanAuthorizationEntry.fromXDR(result.signedAuthEntry, 'base64');
            const creds = signedEntry.credentials();
            console.log('[sdkSigner] credentials type:', creds.switch().name);
            ...
            console.log('[sdkSigner] signature ScVal type:', sig.switch().name);
            
            // The signature should be a Bytes ScVal
            if (sig.switch().name === 'scvBytes') {
              const sigBytes = sig.bytes();
              console.log('[sdkSigner] extracted signature bytes length:', sigBytes.length);
              if (sigBytes.length !== 64) {
                throw new Error(`Expected 64-byte signature, got ${sigBytes.length}`);
              }
              return Buffer.from(sigBytes).toString('base64');
            } else {
              throw new Error(`Unexpected signature type: ${sig.switch().name}`);
            }
          } catch (e) {
            // Fallback: assume it's already the raw signature in base64
            console.error('[sdkSigner] could not extract signature from XDR:', e);
            console.log('[sdkSigner] attempting to use signedAuthEntry as raw signature');
            const rawBytes = Buffer.from(result.signedAuthEntry, 'base64');
            console.log('[sdkSigner] raw bytes length:', rawBytes.length);
            if (rawBytes.length === 64) {
              return result.signedAuthEntry;
            }
            throw new Error(`Invalid signature length: ${rawBytes.length} (expected 64)`);
          }
        }) as any
      : undefined,
  };
